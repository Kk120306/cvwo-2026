...
type Vote struct {
	...
	UserID      string `gorm:"type:uuid;not null;index:unique_vote,unique"`
	VotableID   string `gorm:"type:uuid;not null;index:unique_vote,unique"`        // references the uuid post or a comment
	VotableType string `gorm:"type:varchar(20);not null;index:unique_vote,unique"` // either on a post or a comment
	VoteType    string `gorm:"type:varchar(20);not null"`                          // either like or dislike
	CreatedAt   time.Time
}
